...
var ModalDownload = createReactClass({
  ...
  computeApiUrl: function (values, initalQS = {}) {
    ...
    if (state.startYear && state.startMonth && state.startDay) {
      let sDate = moment(
        `${state.startYear}/${parseInt(state.startMonth) + 1}/${
          state.startDay
        }`,
        'YYYY/M/D'
      );
    ...
    return (
      <fieldset className="form__fieldset form__fieldset--date">
        <legend className="form__legend">
          {what === 'start' ? 'Start' : 'End'} Date
        </legend>
        <div className="form__group">
          <label htmlFor={`${what}-year`} className="form__label">
            Year
          </label>
          <select
            id={`${what}-year`}
            className="form__control form__control--medium select--base-bounded"
            value={this.state[`${what}Year`]}
            onChange={this.onOptSelect.bind(null, `${what}Year`)}
          >
        ...
        <div className="form__group">
          <label htmlFor={`${what}-month`} className="form__label">
            Month
          </label>
          <select
            id={`${what}-month`}
            className="form__control form__control--medium select--base-bounded"
            value={this.state[`${what}Month`]}
            onChange={this.onOptSelect.bind(null, `${what}Month`)}
          >
        ...
        <div className="form__group">
          <label htmlFor={`${what}-day`} className="form__label">
            Day
          </label>
          <select
            id={`${what}-day`}
            className="form__control form__control--medium select--base-bounded"
            value={this.state[`${what}Day`]}
            onChange={this.onOptSelect.bind(null, `${what}Day`)}
          >
      ...
      <fieldset className="form__fieldset form__fieldset--location">
        ...
        <div className="form__group">
          ...
          <select
            ...
            onChange={this.onOptSelect.bind(null, 'locCountry')}
          >
        ...
        <div className={c('form__group', { disabled: disableArea })}>
          ...
          <select
            ...
            onChange={this.onOptSelect.bind(null, 'locArea')}
          >
        ...
        <div className={c('form__group', { disabled: disableLocation })}>
          ...
          <select
            id="loc-location"
            className="form__control form__control--medium select--base-bounded"
            value={this.state.locLocation}
            onChange={this.onOptSelect.bind(null, 'locLocation')}
          >
  ...
  renderResultCountMessage: function () {
    ...
    if (
      this.state.selectionCount !== null &&
      this.state.selectionCount > this.apiLimit
    ) {
      countMessage = (
        <p style={{ textAlign: 'center' }}>
          The API has a limit of {formatThousands(this.apiLimit)} measurements
          and your query yields {formatThousands(this.state.selectionCount)},
          therefore results will be limited.
        </p>
      );
    }
  ...
  render: function () {
    const measurementsCount = this.props.countries.reduce(
