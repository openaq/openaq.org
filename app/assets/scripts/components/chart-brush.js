...
var Chart = function (options) {
  ...
  const calcFocusHeight = () => _height * 0.7;
  ...
  var xAxis = d3.axisBottom(x).tickPadding(8).tickSize(0);
  // .tickFormat(d3.timeFormat('%H:%M'));
  // Define xAxis function.
  var yAxis = d3.axisLeft(y).tickPadding(8).ticks(5).tickSize(0);

  // Define xAxis brush function.
  var xAxisBrush = d3.axisBottom(xBrush).tickPadding(8).tickSize(0);
  ...
  function chartFn(selection) {
    ...
    var layers = {
      ...
      contextRegion: function () {
        ...
        let contextR = $dataCanvas.selectAll('g.context').data([0]);

        let enter = contextR.enter().append('g').attr('class', 'context');
        ...
        contextR.attr('transform', `translate(${0},${calcContextYPos()})`);
      },
      ...
      contextData: function () {
        ...
        dataContainer.enter().append('g').attr('class', 'data');

        if (!_data) return;

        let contextDataGroups = dataContainer
          .selectAll('g.location-data')
          .data(_data);

        contextDataGroups.exit().remove();

        let circles = contextDataGroups
          .enter()
          .append('g')
      ...
      xAxis: function () {
        ...
        xAx
          .attr(
            'transform',
            `translate(${margin.left},${calcFocusHeight() + margin.top + 8})`
          )
          .call(xAxis);
      },
      ...
      xAxisBrush: function () {
        // Append xAxisBrush.
        // X axis brush.
        let xAx = $svg.selectAll('.x.axis-brush').data([0]);
    ...
    upateSize = function () {
      ...
      brush.extent([
        [0, -4],
        [_width, calcContextHeight() + 8],
      ]);
      $dataCanvas.select('g.brush').call(brush.move, x.range());
    ...
    // -----------------------------------------------------------------
    // INIT.
    $svg = $el.append('svg').attr('class', 'chart').style('display', 'block');
      ...
      let yMax = _(_data)
        .map(l => {
          let max = _(l)
            .filter(
              m =>
                m.date.localNoTZ >= newXDomain[0] &&
                m.date.localNoTZ <= newXDomain[1]
            )
            .maxBy('value');
