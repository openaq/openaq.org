...
export default function NearbyLoc ({ countryData, latestMeasurements, loc, parameters, sources }) {
  let {fetched, fetching, error, data: {results: locMeasurements}} = latestMeasurements;
  if (!fetched && !fetching) {
    return null;
  }
  ...
  if (fetching) {
    ...
    if (locMeasurements.length === 1) {
      intro = <p>There are no other locations in {loc.data.city}, {country.name}. {addIntro ? <br/> : null}{addIntro}</p>;
    } else {
      intro = <p>There are <strong>{locMeasurements.length - 1}</strong> other locations in <strong>{loc.data.city}</strong>, <strong>{country.name}</strong>.
