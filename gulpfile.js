...
gulp.task('images', function () {
  return gulp
    .src(['app/assets/graphics/**/*', OPENAQ_ADDONS.graphicsPath + '/**/*'])
    .pipe(
      $.cache(
        $.imagemin([
...
gulp.task('markdown', function () {
  return gulp
    ...
    .pipe(gutil.buffer())
    .pipe(
      $.markdownToJson(marked, 'content.json', function (data, file) {
...
gulp.task('clean', async function () {
  return del(['.tmp', 'dist', 'assets/content']).then(function () {
    $.cache.clearAll();
  });
});
...
gulp.task(
  ...
  gulp.series(
    ...
    function () {
      ...
      gulp.watch('app/content/**/*', gulp.series('markdown'));
